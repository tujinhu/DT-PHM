...
这样三层之间不是简单地"调用前一层"，而是形成了PHM意义上的**认知演化链**：

- **Level I回答**："**我现在发生了什么？**"（What is wrong with me now?）
- **Level II回答**："**如果继续这样，会发生什么？**"（What will happen if I continue?）
- **Level III回答**："**为了继续安全运行，我应该怎么办？**"（What should I do to remain operational?）

注意，这三个问题正好对应了PHM从**Diagnosis → Prognosis → Recovery**的经典闭环，但你的工作不是分别做三个算法，而是证明：**机载数字孪生提供的Physical Self-awareness，是这一整个PHM能力链能够成立的共同前提。**

我认为，这一点才是真正能够把你的论文从"数字孪生做三个PHM应用"提升到"数字孪生赋予无人机一种机载PHM基础能力"的关键所在，也会让"机载实时运行"这一优势成为整个论文最自然、最有说服力的主线。
...
这一层就是你说的：建立Trusted Internal Physical Reference。然后：**利用它理解自己的健康状态。**这里其实就是PHM里面最经典的一层。但是：实验重点不能只是：Fault Detection。而应该证明：没有Trusted Reference时，机器人不知道：到底是不是自己坏了。有了以后，开始知道自己哪里开始偏离正常。

------

### 建议实验一共三步

#### Experiment 1 Motor Fault Detection

例如：推力下降95%、90%、85%、80，证明：数字孪生可以发现执行器异常。

#### Experiment 2 Sensor Fault Isolation

例如：IMU Drift、GPS Jump、Magnetometer Bias。这里不是分类。而是证明DT能够告诉：到底是Sensor坏了，还是Vehicle坏了。因为Reference独立。

#### Experiment 3 Multiple Fault Diagnosis

例如：Motor，IMU同时发生。看看还能不能定位。这里体现Trusted Reference，开始具有诊断能力。

这里最后输出一句话：机器人知道自己哪里坏了。

------

# Level II：Protect Myself

这里建议不要叫：Risk Prediction。建议叫：Predictive Risk Assessment或者：Predictive Action Validation，为什么？因为：预测轨迹很多人做，预测风险别人也做，但是：预测未来动作是否安全，这是DT最天然。整个实验建议围绕：Before Execute，数字孪生提前Rollout。看看未来会不会危险。

------

### Experiment 4 Aggressive Maneuver

例如：Planner要求急转。DT提前模拟。发现未来姿态失稳，于是拒绝。

------

### Experiment 5 Damaged Vehicle

注意：这里开始承接Level I。例如：Level I已经知道：Motor退化。那么同样，Planner给急转。Healthy可以。Degraded不可以。DT提前识别。于是拒绝。这一点非常漂亮。因为，第二层真正利用了第一层输出。不是重新开始。

------

### Experiment 6 Mission Replanning

例如：发现未来高度不足，电量不足，动力不足。DT提前修改轨迹。不是飞完再说，而是提前。这里最后证明机器人开始保护自己。

------

# Level III Save Myself

真正能力应该叫：Operational Continuity，或者Self-preservation。因为机器人已经知道自己坏了，已经预测未来危险。现在开始救自己。这里实验一定要系统级。

------

### Experiment 7 GPS Failure

不是Virtual GPS。而是GPS没了，继续飞。

------

### Experiment 8 IMU Failure

不是估计IMU，而是姿态继续稳定。

------

### Experiment 9 Emergency Landing

例如：Motor、GPS同时。DT接管，安全降落。

------

### Experiment 10 Integrated PHM

这一条我认为应该成为全文最后的大实验。例如：正常飞、Motor开始退化 $\rightarrow$ Level I发现，更新健康状态。Planner继续高速转弯。 $\rightarrow$ Level II预测危险，修改轨迹。随后：GPS失效。 $\rightarrow$ Level III：DT继续传播。安全降落。这一条实验其实就是三个Capability全部串起来。

如果把三层之间真正的数据接口定义清楚，论文会比一般的"三级功能框架"更有理论味道。建议让每一层都**输出一个比上一层更高层次的PHM认知结果**：
