...
if defined RFV_START_INDEX (
    SET /a START_INDEX=%RFV_START_INDEX%
) else (
    SET /a START_INDEX=1
)
...
if defined RFV_VEHICLE_NUM (
    SET /A VehicleNum=%RFV_VEHICLE_NUM%
) else (
    SET /A VehicleNum=2
)
ECHO RFV_VEHICLE_NUM=%RFV_VEHICLE_NUM%
ECHO RFV_START_INDEX=%RFV_START_INDEX%
ECHO VehicleNum=%VehicleNum%
...
REM QGCPath
REM tasklist|find /i "QGroundControl.exe" || start %PSP_PATH%\QGroundControl\QGroundControl.exe -noComPix
