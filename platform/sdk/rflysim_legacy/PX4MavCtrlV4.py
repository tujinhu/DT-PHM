...
class RflySimCP:
    """Minimal RflySim control-protocol constants used by PX4MavCtrlV4."""
...
class RedisKey:
    """ Redis 閫氶亾鐨?Key 绫诲瀷
    閫氳繃绫诲瀷 ID, 鍙互浠巖edisKey涓煡鍑洪€氶亾鐨勫瓧绗︿覆鍚嶇О
    """
    SIL = "SIL"  # 20100
...
class PosTypeMask(object):
    ...
    def __init__(self, ignore_all=False):
        """
        榛樿鍒濆鍖栦负閫熷害+鍋忚埅瑙掗€熺巼妯″紡锛屽綋ignore_all涓篢rue鏃讹紝蹇界暐鎵€鏈塷ffboard鎺у埗閲?
        """
        self.enPos_x = False
        self.enPos_y = False
        self.enPos_z = False
...
class AttTypeMask(object):
    ...
    def __init__(self, ignore_all=False):
        """
            榛樿鍒濆鍖栦负濮挎€?娌归棬妯″紡锛屽綋ignore_all涓篢rue鏃讹紝蹇界暐鎵€鏈塷ffboard鎺у埗閲?
        """
...
class PX4MavCtrler:
    ...
    """鍒涘缓涓€涓€氫俊瀹炰緥
    ID: 濡傛灉ID<=10000鍒欒〃绀洪鏈虹殑CopterID鍙枫€傚鏋淚D>10000锛屼緥濡?0100杩欑锛屽垯琛ㄧず閫氫俊绔彛鍙穚ort銆傛寜骞冲彴瑙勫垯锛宲ort=20100+CopterID*2-2锛堜负浜嗗吋瀹规棫鎺ュ彛鐨勮繃娓″畾涔夛紝灏嗘潵ID鍙〃绀篊opterID锛夈€?
    ip: 鏁版嵁鍚戝鍙戦€佺殑IP鍦板潃銆傞粯璁ゆ槸鍙戝線鏈満鐨?27.0.0.1鐨処P锛涘湪鍒嗗竷寮忎豢鐪熸椂锛屼篃鍙互鎸囧畾192.168鎵撳ご鐨勫眬鍩熺綉鐢佃剳IP锛涗篃鍙互浣跨敤255.255.255.255鐨勫箍鎾湴鍧€锛堜細骞叉壈缃戠粶鍏朵粬鐢佃剳锛?
    Com: 涓嶱ixhawk鐨勮繛鎺ユā寮忋€?
        Com='udp'锛岃〃绀轰娇鐢ㄩ粯璁ょ殑udp妯″紡鎺ユ敹鏁版嵁锛岃繖绉嶆ā寮忎笅锛屾槸鎺ユ敹CopterSim杞彂鐨凱X4鐨凪AVLink娑堟伅锛堟垨UDP_full,simple锛夋秷鎭寘
                 浣跨敤port+1绔彛鏀跺拰port绔彛鍙戯紙渚嬪锛?鍙烽鏈烘槸20101绔彛鏀讹紝20100绔彛鍙戯紝涓嶤opterSim瀵瑰簲锛夈€?
        Com='COM3'锛圵idnows涓嬶級鎴?Com='/dev/ttyUSB0'锛圠inux绯荤粺锛屼篃鍙兘鏄痶tyS0銆乼tyAMA0绛夛級锛岃〃绀洪€氳繃USB绾匡紙鎴栬€呮暟浼狅級杩炴帴椋炴帶锛屼娇鐢ㄩ粯璁?7600鐨勬尝鐗圭巼銆傛敞鎰忥細娉㈢壒鐜囦娇鐢╬ort鍙ｈ缃紝榛樿port=0锛屼細閲嶆槧灏勪负57600
        Com='Direct'锛岃〃绀篣DP鐩磋繛妯″紡锛堝搴旀棫鐗堟帴鍙ｇ殑鐪熸満妯″紡锛夛紝杩欑妯″紡涓嬩娇鐢ㄤ娇鐢ㄥ悓涓€绔彛鏀跺彂锛堢鍙ｅ彿鏈塸ort璁剧疆锛夛紝渚嬪Com='Direct'锛宲ort=15551锛岃〃绀洪€氳繃15551杩欎竴涓鍙ｆ潵鏀跺彂鏁版嵁
        娉ㄦ剰锛欳OM妯″紡鍜孌irect妯″紡涓嬶紝ID鍙〃绀洪鏈虹殑ID鍙凤紝鑰屼笉琛ㄧず绔彛鍙?
        Com='redis':浣跨敤Redis妯″紡閫氫俊锛屾湇鍔″櫒鍦板潃涓篿p锛屾湇鍔″櫒绔彛涓簆ort
    port: UDP妯″紡涓嬮粯璁ゆ儏鍐典笅璁句负0锛屼細鑷姩鏍规嵁IP濉厖锛屾寜骞冲彴瑙勫垯锛宲ort=20100+CopterID*2-2銆傚鏋滆繖閲岃祴鍊煎ぇ浜?锛屽垯浼氬己鍒朵娇鐢╬ort瀹氫箟鐨勭鍙ｃ€?
          COM妯″紡涓嬶紝Port榛樿琛ㄧず娉㈢壒鐜噑elf.baud=port銆傚鏋減ort=0锛屽垯浼氳缃畇elf.baud=57600
          Direct妯″紡涓嬶紝Port榛樿琛ㄧず鏀跺彂绔彛鍙凤紙浣跨敤鐩稿悓绔彛锛?
          redis妯″紡涓嬶紝Port瀵瑰簲鏈嶅姟鍣ㄧ鍙ｅ彿self.redisPort = port銆傚鏋減ort=0锛屽垯self.redisPort=6379涓哄钩鍙伴粯璁ゅ€笺€?

    鎺ュ彛绀轰緥锛?
    UDP妯″紡
    PX4MavCtrler(1) # 榛樿IP
    PX4MavCtrler(1,'192.168.31.24') # 鎸囧畾IP锛岀敤浜庤繙绋嬫帶鍒?

    涓插彛妯″紡
    PX4MavCtrler(1,'127.0.0.1','com1',57600) # 鎸囧畾涓插彛锛屽苟璁剧疆娉㈢壒鐜?
    
    PX4MavCtrler(1,'192.168.31.105','Direct',15552) # 鐪熸満鐩磋繛Direct鏂瑰紡锛岃繙绔疘P涓?192.168.31.105'锛岀鍙?5552

    """
    """
    娉ㄦ剰锛氫笅闈㈣繖閲屾槸鏃х増鎺ュ彛锛屼究浜庣敤鎴峰弬鑰冩棫瑙勫垯杩涜鎺ュ彛鍗囩骇
    ...
    def __init__(self, ID=1, ip='127.0.0.1',Com='udp',port=0, simulinkDLL=False):
        ...
        # 杩欓噷鏄负浜嗗吋瀹逛箣鍓嶇殑PX4MavCtrler('COM3:115200')涓插彛鍗忚锛屽皢鏉ヤ細鍙栨秷
        if type(ID) == str: #濡傛灉ID鏄瓧绗︿覆杈撳叆
            Com=ID
            ID=1
        ...
        # UDP妯″紡瑙ｆ瀽
        if (Com=='udp' or Com=='UDP' or Com=='Udp') and ID>10000: # 濡傛灉鏄疷DP閫氫俊妯″紡
            # 鍏煎鏃х増鍗忚锛屽鏋淚D鏄?0100绛夌鍙ｈ緭鍏ワ紝鍒欒嚜鍔ㄨ绠桟opterID
            self.port=ID
            self.CopterID = int((ID-20100)/2)+1

        # 涓插彛杩炴帴妯″紡瑙ｆ瀽
        self.ComName = 'COM3' # 榛樿鍊硷紝涓插彛鍚嶅瓧

        if Com[0:3]=='COM' or Com[0:3]=='com'  or Com[0:3]=='Com' or Com[0:3]=='/de': # 濡傛灉鏄覆鍙ｈ繛鎺ユ柟寮?
            self.isCom = True # 涓插彛閫氫俊妯″紡
            strlist = Com.split(':')
            if port==0: # 榛樿鍊?7600
                self.baud = 57600
            else:
                self.baud = int(port)
            if(len(strlist) >= 2): # 涓插彛鍙?娉㈢壒鐜?鍗忚瑙ｆ瀽锛屼负浜嗗吋瀹规棫鎺ュ彛
                if strlist[1].isdigit():
                    self.baud = int(strlist[1])
            self.ComName = strlist[0] # 涓插彛鍚嶅瓧

        # 缃戠粶鐩磋繛妯″紡瑙ｆ瀽
        if Com[0:6]=='Direct' or Com[0:6]=='direct': # 濡傛灉UDP鐩磋繛鐨勭湡鏈烘ā寮?
            self.port = int(port)
            strlist = Com.split(':')
            if(len(strlist) >= 2):
                if strlist[1].isdigit():
        ...
        if Com[0:5]=='redis' or Com[0:5]=='Redis' or Com[0:5]=='REDIS':
            ...
            if isEnableRdis:
                ...
                self.pubsub = None
                self.redisKey = {
                    RedisKey.SIL: RedisKey.GetRedisKey(self.port),  # 鎺у埗绔彛锛?0100+2*(copterId-1)
                    RedisKey.SIL_TRUE: RedisKey.GetRedisKey(self.port + 10000),  # 鎺у埗绔彛锛?0100+2*(copterId-1)
                    RedisKey.R3D: "GLOBAL_MESSAGE_REDIS_KEY",  # R3D: R3D 鎺ユ敹澶栭儴杈撳叆鐨?redis key
                    RedisKey.SIL_RECV: RedisKey.GetRedisKey(self.port + 1),  # 鍥炰紶绔彛锛?0101+2*(copterId-1)
                    RedisKey.SIL_TRUE_RECV: RedisKey.GetRedisKey(self.port + 10000 + 1),  # 鍥炰紶绔彛锛?0101+2*(copterId-1)
                }
        ...
        self.uavActuatorOutput = [0, 0, 0, 0, 0, 0]

        self.truePosGPS = [0, 0, 0] # True simulated PosGPS from CopterSim's DLL model
        self.trueSimulinkData=[0]*32 #create 32D data锛屾潵鑷狣LL妯″瀷鐨凮utCopterData鎺ュ彛
        ...
        self.offMode=0
        self.ctrlMode=0 # Simple妯″紡涓嬮鏈虹殑椋炶妯″紡
    ...
    def sendUE4PosNew(
        ...
        #     int PosGpsInt[3];   //lat*10^7,lon*10^7,alt*10^3锛宨nt鍨嬪彂鏀捐妭鐪佺┖闂?
        #     float MotorRPMS[8];
        #     float VelE[3];
        ...
        # print(len(buf))
        if windowID < 0:
            for i in range(3):  # 鍋囪鏈€澶氬紑浜嗕笁涓獥鍙?
    ...
    # //杈撳嚭鍒癈opterSim DLL妯″瀷鐨凷ILints鍜孲ILFloats鏁版嵁
    ...
    def EndTrueDataLoop(self):
        ...
        if not self.simulinkDLL:
            ...
            # join闇€瑕佸姞涓€涓弬鏁帮紝鍚﹀垯浼氫竴鐩撮樆濉炲湪姝?
            self.tPX4.join(1)
    ...
    def InitMavLoop(self,UDPMode=2):
            ...
            if self.isCom:
                ...
                if self.isRealFly:
                    ...
                    if self.isRedis:
                        ...
                        # 鍐嶅惎鍔ㄤ竴涓嚎绋嬶紝鍗曠嫭鎺ユ敹Redis Mavlink娑堟伅
                        # 鏀跺埌redis鍖呬箣鍚庯紝绔嬪埢杞彂buf鍒?27.0.0.1 , self.port+1
        ...
        self.isFailsafeEn = False
        self.SendMavArm(False) # 鍙戦€佷竴鏉′笂閿佸懡浠ょ粰CopterSim
    ...
    def SendVisionPosition(self, x, y, z, yaw):
        """
        x,y,z 鏄姩鎹曟垨瑙嗚娴嬮噺鐨勯鏈轰綅缃紝yaw鏄姩鎹曟垨瑙嗚娴嬮噺鐨勫亸鑸
        """
        ...
        if self.isCom or self.isRealFly:
            ...
            if self.UDPMode>1.5:
                ...
                if self.UDPMode==0:
                    ...
                    elif self.offMode == 13: # 濡傛灉鏄€熷害楂樺害鍋忚埅鍥哄畾缈兼ā寮?
    ...
    def SendAccPX4(self,afx=0,afy=0,afz=0,yawValue=0,yawType=0,frameType=0):
        ...
        self.offMode=0 # SET_POSITION_TARGET_LOCAL_NED
        self.ctrlMode=8 #鍔犻€熷害鎺у埗妯″紡
        if yawType==0:
            self.EnList = [0,0,1,1,0,0]
            self.yaw = 0
            self.yawrate = 0
            self.ctrlMode=6 #鍔犻€熷害鎺у埗妯″紡锛屾棤鍋忚埅鎺у埗
        elif yawType==1:
            self.EnList = [0,0,1,1,1,0]
            self.yaw = yawValue
            self.yawrate = 0
            self.ctrlMode=7 #鍔犻€熷害鎺у埗妯″紡锛屽亸鑸鎺у埗
        else:
            self.EnList = [0,0,1,1,0,1]
            self.yaw = 0
            self.yawrate = yawValue
            self.ctrlMode=8 #鍔犻€熷害鎺у埗妯″紡锛屽亸鑸閫熷害鎺у埗
    ...
    def SendVelYawAlt(self,vel=10,yaw=6.28,alt=-100):
        ...
        if abs(yaw)<0.00001:
            yaw = 6.28
        self.offMode=0 # SET_POSITION_TARGET_LOCAL_NED
        self.ctrlMode=13 #閫熷害楂樺害鍋忚埅鎺у埗妯″紡
    ...
    def LandSyn(self, height=0):
        """Send one land command without delay."""
        self.ResetSynCmd()
    ...
    def SendDTIMUCtrlMsg(self, mode, ctrls=[0]*16):
        time_boot_ms = int((time.time()-self.startTime)*1000)
        controls = self.fillList(ctrls,16,0)
        
        if self.isCom or self.isRealFly:
            self.the_connection.mav.rfly_dt_imu_send(time_boot_ms,mode,controls)
        else:
            buf = self.mav0.rfly_dt_imu_encode(time_boot_ms,mode,controls).pack(self.mav0)
            self.SendBuf(buf)
        #print("Msg Send.")
    ...
    def getTrueDataMsg(self):
        ...
        while True:
            ...
            try:
                ...
                # struct Ue4CMDGPS{
                #     int checksum;//鏍￠獙鐮侊紝鐢ㄤ簬纭鏁版嵁姝ｇ‘鎬э紝杩欓噷鍙栧€?1234567890
                #     int CopterID;
                #     double GPS[3];
                # } ii3d
                if len(buf)==32:
                    ...
                    if checksum==1234567890:
                        if not self.useCustGPSOri:
                            ...
                            # 濡傛灉鑾峰彇鍒颁簡GPS鍧愭爣锛屽垯閲嶆柊璁＄畻浣嶇疆鍋忓樊
        ...
        while True:
            ...
            if self.UDPMode>1.5: # If Use MAVLink Mode
                ...
                if msg is not None:
                    ...
                    if msg.get_type() == "ACTUATOR_OUTPUT_STATUS":
                        self.uavActuatorOutput[0] = msg.actuator[0]
                        self.uavActuatorOutput[1] = msg.actuator[1]
                        self.uavActuatorOutput[2] = msg.actuator[2]
                        self.uavActuatorOutput[3] = msg.actuator[3]
                        self.uavActuatorOutput[4] = msg.actuator[4]
                        self.uavActuatorOutput[5] = msg.actuator[5]
                    ...
                    if msg.get_type() == "LOCAL_POSITION_NED":
                        ...
                        self.netForwardData() # 灏嗘秷鎭浆鍙戝嚭鍘?
                ...
                if self.UDPMode==0:
                    ...
                    #     int32_t GpsPos[3];     //Estimated GPS position锛宭at&long: deg*1e7, alt: m*1e3 and up is positive
                    ...
                    try:
                        ...
                        if len(buf)==52:
                            ...
                            #    int checksum;
                            #    int32_t gpsHome[3];     //GPS鍘熷鏁版嵁锛屽叾涓粡绾害(搴?1e7)锛岄珮搴﹀悜涓婁负姝?鍗曚綅m*1e3->mm)
                            #    float AngEular[3];    //婊ゆ尝鍚庣殑椋炴満娆ф媺瑙掞紝鍗曚綅rad
                            #    float localPos[3];    //婊ゆ尝鍚庣殑鏈湴浣嶇疆锛屽崟浣峬
                            #    float localVel[3];    //婊ゆ尝鍚庣殑鏈湴浣嶇疆锛屽崟浣峬/s
                            #}
    ...
    # 娉ㄦ剰锛氳繖涓帴鍙ｅ皢鏉ヨ绉绘鍒癉llSimCtrlAPI.py閲岄潰鍘?
    # 鍙戦€佸埌DLL妯″瀷鐨刬nSILInts and inSILFLoats鎺ュ彛
    #    struct PX4SILIntFloat{
    #     int checksum;//1234567897
    #     int CopterID;
    #     int inSILInts[8];
    #     float inSILFLoats[20];
    #    }
    # inSILInts and inSILFloats will be forwarded to simulation model inputs.
